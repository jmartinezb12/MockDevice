...
class AIDProvider (private val context: Context){
    companion object {
        var aidList :MutableSet<AidData> = Collections.synchronizedSet(mutableSetOf())
    }
    private fun loadAIDs(): Set<AidData> =
        loadAidsFromJson().ifEmpty { getDefaultAids() }

    fun getAIDS(mandatory: Boolean): Set<AidData> {
        Log.i("AIDS", "Verificando si es mandatorio....")
        synchronized(aidList){
        ...
        return aidList.toSet()
    }

    private fun loadAidsFromJson(): Set<AidData> {
        return try {
            val json = context.assets.open("aids.json").bufferedReader().use { it.readText() }
            val type = object : TypeToken<Set<AidData>>() {}.type
            val parsedJson: Set<AidData> = Gson().fromJson(json, type)
    ...
    fun getDefaultAids(): Set<AidData> {
        return setOf(
